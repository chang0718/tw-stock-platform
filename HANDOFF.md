...
1. **UI 整體 RWD**：P1 已於 2026-07-09 完成 —— app.py 全域 CSS 加 `@media(max-width:640px)`：`st.columns` 自動換行（涵蓋所有多欄指標卡含技術面 6 欄信心）、`.sc-row` 可橫向捲動、縮小留白/字體。P2（`layout` 條件切換、原生 mobile 設定）仍待辦。
2. **Tab 4 追蹤清單技術面**：首次需手動點載入按鈕，可考慮自動預載
3. ~~**毛利率/淨利率**：仍取最新單季~~ → ✅ 已於 2026-07-08 F1 改 TTM
...
6. ~~**外資持股趨勢加到追蹤清單籌碼頁**~~ → ✅ 2026-07-09 完成（追蹤頁「📊 基本面/籌碼」子頁 render_flow_block 後，lazy 按鈕載入 render_major_holder_block）
7. ~~**TDCC 大戶週資料雲端長期累積**~~ → ✅ 2026-07-09 完成：歷史檔移 tracked `data/major_holders/history.json`；新 `scripts/tdcc_snapshot.py`（股票池=config 供應鏈/概念股 ∪ 紙上持倉 ∪ 現有歷史，`TDCCLoader.bulk_snapshot` 一次寫入、日期去重冪等）；workflow 加排程步驟 + commit-back。`get_major_holders` 改**唯讀**（併入本週供顯示，不寫 tracked 檔），寫入僅由排程負責。
