...
1. **UI 整體 RWD**：目前無 mobile 支援，寬度固定（`layout="wide"` + 固定 px CSS + 硬編 `st.columns`）→ 手機需橫向捲動、指標卡擠壓。P1 兩項（`.sc-*` 改 %/max-width、關鍵 columns 加 `@media` 斷點）見 `docs/REVIEW_2026-07.md` D 節。
